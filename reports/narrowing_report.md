# Narrowing Report — 2026-05-09 23:34:04

## Overview

- Parent NARROW topics processed: **10**
- Total candidate variants generated: **18**
- Variants selected for re-run: **4**
...
|---|------------|--------|----------------|-------------|-------------|-----------|----------|----------|-----------|
| 1 | T53_N4 | T53 | noise_pruned | 0.500 | +0.000 | 25 | 4 | 2 | 0.6250 |
| 2 | T07_N1 | T07 | noise_pruned | 0.500 | +0.000 | 13 | 2 | 2 | 0.6050 |
| 3 | T37_N1 | T37 | noise_pruned | 0.562 | +0.312 | 2 | 1 | 3 | 0.5567 |
| 4 | T74_N1 | T74 | noise_pruned | 0.500 | +0.250 | 1 | 1 | 3 | 0.5550 |

## Per-Parent Analysis

### T04: Prompt-template sensitivity benchmark for clinical-domain LLM judges

**Original signals**: purity=0.25, citation=4, artifact=2
...
|------------|------|--------------------------|-------------|-------------|------|
| T04_N1 | noise_pruned | `LLM judge|prompt template sensitivity` | 0.000 | +0.000 |  |
...
| Variant ID | Type | New keywords (truncated) | Est. purity | Purity gain | Sel? |
|------------|------|--------------------------|-------------|-------------|------|
| T07_N1 | noise_pruned | `prompt injection|LLM judge` | 0.500 | +0.000 | ✓ |
...
- **Negatives**: `general red teaming;adversarial robustness`
- **Narrowing note**: removed noisy secondaries=['adversarial robustness']; added to negatives
- **Est. relevance purity**: 0.500 (parent: 0.500, gain: +0.000)
- **Est. kept papers**: 13 (hi-relevance: 13)
- **Est. citation_signal**: 2/5
- **Est. artifact_opportunity**: 2/5
- **Est. NIW/EB1A/Career**: 4/4/5
- **Est. novelty risk**: MEDIUM
- **Composite score**: 0.6050
...
| Variant ID | Type | New keywords (truncated) | Est. purity | Purity gain | Sel? |
|------------|------|--------------------------|-------------|-------------|------|
| T10_N1 | noise_pruned | `LLM-as-a-judge|methodology disclosure` | 0.200 | +0.000 |  |

**Selected variant details:**

### T11: Format sensitivity benchmark on LLM evaluations

**Original signals**: purity=0.25, citation=3, artifact=4
...
| Variant ID | Type | New keywords (truncated) | Est. purity | Purity gain | Sel? |
|------------|------|--------------------------|-------------|-------------|------|
| T17_N1 | primary_anchor | `tokenizer understanding generation|leaderboard variance|eval…` | 0.000 | +0.000 |  |
| T17_N2 | primary_anchor | `tokenizer visual generation|leaderboard variance|evaluation …` | 0.000 | +0.000 |  |
| T17_N3 | primary_anchor | `tokenizer unified image|leaderboard variance|evaluation nois…` | 0.000 | +0.000 |  |
| T17_N4 | primary_anchor | `tokenizer multimodal understanding|leaderboard variance|eval…` | 0.000 | +0.000 |  |
...
| Variant ID | Type | New keywords (truncated) | Est. purity | Purity gain | Sel? |
|------------|------|--------------------------|-------------|-------------|------|
| T37_N1 | noise_pruned | `clinical guideline|model drift|LLM versioning` | 0.562 | +0.312 | ✓ |

**Selected variant details:**

#### T37_N1 — noise_pruned
- **Title**: Clinical-guideline consistency across LLM versions over time — noise-pruned
- **Keywords**: `clinical guideline|model drift|LLM versioning`
- **Synonyms**: `USPSTF;CDC guidelines;NICE;medical advice consistency`
- **Negatives**: `drug discovery;longitudinal evaluation`
- **Narrowing note**: removed noisy secondaries=['longitudinal evaluation']; added to negatives
- **Est. relevance purity**: 0.562 (parent: 0.250, gain: +0.312)
...
Candidates generated: 5 | Selected: 1

| Variant ID | Type | New keywords (truncated) | Est. purity | Purity gain | Sel? |
|------------|------|--------------------------|-------------|-------------|------|
| T53_N4 | noise_pruned | `data contamination|benchmark leakage|PubMedQA|MMLU clinical` | 0.500 | +0.000 | ✓ |
| T53_N5 | synonym_promoted | `memorization|data contamination|benchmark leakage` | 0.250 | +0.000 |  |
| T53_N1 | primary_anchor | `data contamination static dynamic|benchmark leakage|PubMedQA` | 0.250 | +0.000 |  |
| T53_N2 | primary_anchor | `data contamination dynamic evaluation|benchmark leakage|PubM…` | 0.250 | +0.000 |  |
| T53_N3 | primary_anchor | `data contamination static dynamic evaluation|benchmark leaka…` | 0.250 | +0.000 |  |
...
- **Est. relevance purity**: 0.500 (parent: 0.500, gain: +0.000)
- **Est. kept papers**: 25 (hi-relevance: 24)
...
- **Est. citation_signal**: 1/5
- **Est. artifact_opportunity**: 3/5
- **Est. NIW/EB1A/Career**: 2/5/4
- **Est. novelty risk**: LOW
- **Composite score**: 0.5550
